...
```
角色设定：你是一个精通 WebGL (Three.js) 和计算机视觉 (MediaPipe) 的高级前端工程师。
任务目标： 请开发一个基于 Web 的互动 3D 粒子系统项目，核心是一棵由数万个粒子组成的 3D 圣诞树。该系统需要能够通过摄像头捕捉用户的手势和面部表情来实时改变圣诞树的形态和颜色。
核心功能要求：
1. 3D 粒子渲染 (Three.js)：
    * 创建一个包含 50,000+ 个粒子的 3D 场景。
    * 使用 THREE.Points 和自定义 ShaderMaterial 来渲染粒子，实现高性能和发光效果。
    * 模型形态：核心模型为圣诞树 (Christmas Tree)，粒子需要组成清晰的树干、三层树叶圆锥和装饰点缀。
    * 动态效果：粒子需要有轻微的漂浮动画，且在状态切换时通过 Lerp 插值平滑过渡。
2. 手势交互 (MediaPipe Hands)：
    * 集成 MediaPipe Hands 库，通过摄像头实时追踪用户手部。
    * 功能：计算手掌的开合程度（0% 握拳 - 100% 张开）。
    * 交互逻辑：手掌的开合程度直接控制圣诞树的大小/缩放（握拳变小，张开变大）。
3. 情绪识别交互 (MediaPipe Face Mesh)：
    * 集成 MediaPipe Face Mesh 库，实时分析用户面部特征点。
    * 识别以下 5 种情绪，并改变粒子的颜色主题：
        * 😐 常规 (Neutral)：全树呈现梦幻的 白色 + 柔和粉色 风格（不显示顶部黄星）。
        * 😄 开心 (Happy)：恢复圣诞树经典的 多彩原色（绿叶、棕干、彩灯、金顶）。
        * 😲 惊讶 (Surprise)：变为 阳光黄 + 活力橙 + 热粉色 的明亮暖色调。
        * 😠 生气 (Angry)：变为 红色 + 橙色 的警示色调（需优化眉毛识别阈值以便于触发）。
        * 😢 悲伤 (Sad)：变为 午夜深蓝 + 钢蓝色 的冷色调。
4. UI 界面：
    * 左上角显示：实时的手势开合度百分比 (✋ Hand Open/Close)。
    * 右上角显示：当前识别到的情绪状态 (😄 Emotion)。
    * 控制面板 (GUI)：包含 "Base Color"（基础色）、"Particle Count"（粒子数）、"Auto Rotate"（自动旋转）、"Toggle Fullscreen"（全屏）。
    * 注意：不提供模型形状切换功能，仅专注于圣诞树。
5. 技术栈与规范：
    * 前端框架：Vanilla JS (ES6模块化) 或简单的 HTML/CSS/JS 结构。
    * 库：Three.js (渲染), MediaPipe (视觉), lil-gui (控制面板)。
    * 风格：背景深色 ( #050505 )，粒子发光，整体呈现现代科技感与节日氛围的结合。
